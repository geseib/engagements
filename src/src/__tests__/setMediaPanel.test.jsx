...
describe('picking a folder uploads it, with no second button to press', () => {
  ...
  test('a file the server refuses is named with the server’s reason', async () => {
    mockApi({
      presign: {
        uploads: [],
        rejected: [{ name: 'notes.jpg.exe', reason: '"notes.jpg.exe" is a .exe file. Only images are accepted here.' }],
      },
    });

    render(<SetMediaPanel setId={SET_ID} />);
    await screen.findByTestId('smed-summary');
    // Passes the client's own extension check by carrying .jpg in the middle;
    // the server's answer is the one that counts.
    pick([file('notes.jpg')]);

    // The presign response named a different file, so nothing was uploaded and
    // the panel does not claim otherwise.
