...
describe('the editor receives both halves', () => {
  ...
  // rejects: the editor opening on the rewrite as if it were the saved text,
  // so Cancel closed without asking and the applied fixes — a paid model call
  // — were gone. Unsaved is measured against the prompt as it is STORED.
  test('the rewrite counts as unsaved: Cancel asks before throwing it away', async () => {
    ...
    expect(screen.getByTestId('prompt-input-textarea')).toHaveValue(REWRITE.instructions);
  }, 20000);
});
