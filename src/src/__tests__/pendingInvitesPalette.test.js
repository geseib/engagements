/**
 * THE INVITATION PROMPT'S CSS CONTRACT — components/PendingInvites.css.
 *
 ...
 * an unanchored `*token*`: a file named pendingInvitesTokens.test.js runs locally,
...
const MY_CSS = read('components', 'PendingInvites.css');
const MY_JSX = read('components', 'PendingInvites.jsx');
...
describe('the rules that are about what must NOT be there', () => {
  ...
  test('no hex literal survives outside the token block', () => {
    const declarations = stripped(MY_CSS)
      .split('}')
      .filter((blk) => !/^\s*\.pinv\s*\{/.test(blk))
      .join('}');
    expect(declarations.match(/#[0-9a-fA-F]{3,8}\b/g) || []).toEqual([]);
  });

  // rejects: a global class. `.pinv` is the whole namespace.
  test('every selector is rooted at the scope class', () => {
    for (const blk of stripped(MY_CSS).split('}')) {
      ...
      for (const sel of head.split(',')) {
        ...
        if (!trimmed) continue;
        expect(trimmed.startsWith('.pinv')).toBe(true);
      }
    }
  });
  ...
  test('styles.css declares nothing in this scope', () => {
    expect(GLOBAL_CSS).not.toMatch(/(^|[\s,>+~])\.pinv\b/);
  });
});

describe('a refusal stays readable', () => {
  // rejects: painting the server's refusal in a colour that fails on the tint
  // it sits on. --danger is 4.38:1 on --surface, which is why --danger-text
  // exists and why this is asserted rather than assumed.
  test('the error line clears AA on the field', () => {
    expect(on(token(GLOBAL_CSS, ':root {', '--danger-text'), [T.bg]))
