...
**Optionally, pre-warm one beat earlier.** The host typically talks for 30–120 seconds over the final RESULTS screen before advancing. Category counts are decremented when each round is asked (`next-question.js`, `decrementCategoryCount`; get-results' copy, which never ran without throwing, was removed 2026-09-25), so "no questions remain" is knowable at the final RESULTS, not only at the ENDED transition. Speculatively firing there buys most of the latency back. Keep the ENDED trigger as the guaranteed path regardless; the Put is idempotent.
