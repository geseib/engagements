...
- **D11 · Selection is keyed by the editor row's `uid`**, the one key that survives an edit and exists for unsaved rows.
  **Extended after the branch's review:** a uid does not survive a Save. The panel reads the set back and `toRow` mints a new uid for every row, so the preview now stays mounted through that read-back and finds its question again. It does not use the key the question had before the save. `upload-questions.js` renumbers on every save (categories by first appearance, questions within their category), so after removing, moving or re-categorising a question, its old key can belong to a different question. Position alone is wrong too, because the read-back comes in key order and groups each category together. So `utils/questionRows.js:savedKeys` mirrors the importer's numbering over the working copy, and the preview looks for the read-back row under that key with the same title (`config/questionPreview.js:refindPlace`). It falls back to the same position only when no row is found. `tests/question-set-roundtrip.js` holds `savedKeys` to the keys the real importer writes.
