...
- **Reuse `docs/design/admin-redesign/audit.html`** — 6 assertions × 22 mockups × 2 viewports, every check demonstrated failing before it was trusted. The checks are pure functions over a rendered document plus a viewport, so they port into component tests unchanged. **Note the filename:** this set is the only one of the four whose checks live *inline in a `<script>` block in `audit.html`* — `entry-`, `host-` and `player-redesign` each carry a separate `audit.js`, and earlier drafts of this plan copied that pattern and cited an `admin-redesign/audit.js` that has never existed. Reusing these means lifting `rgb` / `lin` / `lum` / `ratio` / `alphaOver` / `bgOf` / `visible` / `hasOwnText` and the `// A4 — contrast` branch of `run()` out of the HTML, not importing a module. A4 (WCAG AA against the *composited* background) matters most here: the Question sets tab is still `contentTheme: 'light'` (`AdminPage.jsx:41-48`), and a converted panel dropped onto the dark field without its own palette pass is `#333` on `#0F1A2E` — 1.4:1. Convert theme and markup in the same commit, and add a `questionSetsPalette.test.js` beside the existing `adminTabsPalette.test.js` and `adminShellPalette.test.js`.
