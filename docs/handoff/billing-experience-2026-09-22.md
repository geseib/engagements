...
| 3. Adjustments and codes | **Shipped to dev 2026-09-22.** `shared/pricing-adjust.js` (pure; copied to `game/`, and to `websocket/` since 2026-09-23; the order of application), `orgs/adjustments.js` (grant/list/revoke, codes create/list/retire, `redeemCodeItems` used by plan-request approval in the same transaction — a declined request burns no use), `get-usage.js` returns `adjusted`. Frontend: `AdjustmentsLedger.jsx` (one list for both sides + the bill in miniature), `OrgBillingDrawer.jsx` (drawer, grant, revoke), `DiscountCodesPanel.jsx` (section `discountcodes`), Plan & usage shows the adjusted bill and the ledger. Tests: `tests/pricing-adjust.js`, `tests/adjustments-ledger.js`, `src/__tests__/adjustments.test.jsx`, `adjustmentsLedgerPalette.test.js`. Mockups 16–19. |
| 4. Simulated invoices | **Shipped to dev 2026-09-22.** `shared/invoices.js` (`closeInvoice`, called by `usage-reconcile.js` for the previous period nightly; conditional put; credits drawn down in the same transaction with `CREDIT_APPLIED` ledger rows; `settle()` is the one Stripe seam). `get-usage.js` serves `GET /orgs/{id}/invoices` and `/invoices/{period}` from the frozen rows. Frontend: `InvoicePanel.jsx` (Billing history, the invoice with the banner twice and every `$` a button with `data-source`, the row dialog) as places inside the Billing section. Tests: `tests/invoice-close.js`, `src/__tests__/invoicePanel.test.jsx`, palette sheet added to `adjustmentsLedgerPalette.test.js`. Mockups 20–21. **All four steps are built.** |
| Metering (found 2026-09-23) | **The session meter was never called.** `recordBillableSession` had no call site from `6730ce9a` (2026-08-23) on, so no `LEDGER#…#SESSION#` row was ever written, `sessionsRun` stayed 0 for every org, the free plan's session cap could not fire, and invoices counted no sessions. The "2 of 5" seen on Plan & usage in the 2026-09-22 walk was the **sets** meter (`setsCurrent`, fed by `usage-stream.js` + the reconciler), not sessions. Wired in `game/join-game.js` on the first join (4b39c871), then **moved the same day by the owner's ruling**: a session counts at the **first answer to its second answered question** ("the session only counts if at least 2 questions get answered by 1 or more people"). `websocket/session-count.js`, behind the answer write in `websocket/message.js`, is the only caller; METADATA carries `FirstAnsweredRound` and `CountedAt`. Creating, starting, joining, unanswered or skipped questions and one answered question are free. Billed once (conditional put) to the session's own org; no org bills nobody; a meter failure never loses the answer and the next answer retries. Hosts read the rule in the help guide `host-plan` (`/help/billing`) and on Plan & usage beside Sessions run. Test: `tests/billable-session-wiring.js`. Not recoverable: sessions run before the fix have no ledger rows, and invoices already closed are frozen (never backfill). |
| Fix: extra allowance reaches the gates | **Fixed 2026-09-23.** A staff grant of extra allowance (`CREDIT_UNITS`) was on the bill but invisible to both gates: `readAllowance` computed `allowanceState(planFor(orgRow), usage)` from the bare plan, so a free org granted +5 sessions read "10 included" on Plan & usage and was still refused its sixth session (402 from `websocket/create-game.js`) and its sixth set (`admin/upload-questions.js`). It stayed latent until `recordBillableSession` was wired into `game/join-game.js` the same day — before that `sessionsRun` never rose. `readAllowance` now also Queries `ORG#<org>/ADJ#` and gates on `effectivePlan(planFor(orgRow), rows, period)` — the function the bill uses, with pricing-adjust's own `isActive` deciding which grants count (revoked, expired and not-yet-started rows do not). An unreadable ledger fails **open** (`reason: 'adjustments-unreadable'`), like the other two reads. `pricing-adjust.js` is now in all three bundles (`websocket/` added, because `usage.js` requires it there); `tests/pricing-adjust.js` pins three copies. `get-question-sets.js`'s `setAllowance` (the editor's "no room for a copy" warning) reads the same function, so it counts the grant too. `RATE_OVERRIDE` changes only prices, so it never moves a gate. No IAM change: all three callers already hold `DynamoDBCrudPolicy`. Tests: `tests/plan-gating.js` §5 (grants through the real staff route; +5 sessions allowed at the 6th and 10th and refused at `used 10 / included 10`, which is get-usage's `adjusted.plan.includedSessions`; revoked, expired and upcoming grants ignored; a sets grant lifts the set gate only; an unreadable ledger fails open), each check watched failing against a deliberately broken gate. Still open: `get-usage.js` and `shared/invoices.js` read `ADJ#` with a single unpaginated Query, while the gate paginates. They would disagree only past 1 MB of adjustment rows for one org. |
| Plan-limit notice (2026-09-23) | **Every refusal says why and who can fix it.** Mockup `docs/design/tenancy-redesign/22-plan-limit-notice.html`. Server: every plan-limit 402 (create-game, upload-questions, and now copy-question-set, which had NO set gate) carries a `resolve` block from `shared/plan-limit.js` (copied to `websocket/`): the caller's role, `canRequest` (owner only, as plan-requests.js authorises), `canViewBilling`, the org, `contacts` (owners for an admin; owners then admins for a member; never the caller), a waiting Team-plan `request`, `resetsOn`. Looked up only on the refusal path; a failed lookup still answers 402. `get-question-sets.js` adds it to `setAllowance` when a copy would be refused. An AI-builder job refused at the allowance keeps the whole 402 as `setCreationLimit` (generated-set.js). Client: `utils/planLimitCopy.js` (the words, pure) and `components/PlanLimitNotice.jsx` (`.plim`, dusk or paper) replace Quickstart's `alert()`, "Upload failed:", "Save failed:", "Could not create…" and the console's link-less text, in: the New session dialog, Quickstart, the upload panel, BuilderPage, the console's Question sets list and Public library (which had no notice at all), the editor's on-arrival warning, its New set dialog, and the AI builders' job panel. `/admin?section=billing&request=team` opens the request dialog. Tests: `tests/plan-limit-resolve.js`, `generated-set-creation.js` §limit; jest `planLimitCopy`, `planLimitNotice`, `planLimitNoticePalette`, `adminPlanLimitWiring`, `builderPageLimit` and the call-site suites. Not done: refusing an AI generation BEFORE it is paid for when there is no room for its set. |
...
- `readAllowance(orgId)` (`:388-437`): Get `ORG#<org>/METADATA`, Get usage, `allowanceState(planFor(orgRow), usage)`. **Fails open** on any read error or missing METADATA (`:397-421`). *(As audited on 2026-09-22. Since 2026-09-23 it also Queries `ADJ#` and gates on the effective plan — see STATUS, "extra allowance reaches the gates".)*
