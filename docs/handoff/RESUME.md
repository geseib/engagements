...
**`window.location` cannot be mocked** under jsdom 26 — it is unforgeable, so every assignment to `pathname`/`search`/`href` is an ignored navigation. `setupTests.js` used to try, silently failing and printing a navigation error per suite (350 in one dev build); that mock is deleted (2026-09-25) and `setupTests.js` now fails any suite whose setup writes to `console.error`. Use `window.history.pushState` to move the browser and `auth/navigate.js`'s `navigateTo` for destinations.
