...
4. **Nothing is ever billed.** `recordBillableSession` is exported from four
   ...
   **Fixed 2026-09-23:** `websocket/session-count.js` calls it at the first
   answer to a session's second answered question — the owner's rule, which
   replaced the first-join call 4b39c871 made the same morning. Driven end to
   end by `tests/billable-session-wiring.js`. See the billing handoff's
   "Metering" note for what the meters showed before.
