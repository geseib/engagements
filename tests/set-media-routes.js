...
(async function run() {
  ...
  await test('the content type comes from the extension, never from the caller', async () => {
    reset();
    // A caller claiming image/jpeg for an executable gets nothing signed.
    const res = await upload([{ name: 'payload.exe', size: 10, type: 'image/jpeg' }]);
    assert.strictEqual(res.statusCode, 200, res.body);
    assert.strictEqual(body(res).uploads.length, 0);
    assert.strictEqual(signed.length, 0);
    assert.match(body(res).rejected[0].reason, /Only images are accepted/);
  });
